...
#define STRSTR(S, X) { std::ostringstream my_ostringstream; my_ostringstream << X; S = my_ostringstream.str(); }

// sizeof an array in elements
#define NUM_ELEMENTS(a) (sizeof(a)/sizeof(a[0]))

// get a C-array element, return default if out of bounds
#define GET_ELEMENT(ARRAY, IDX, DEFAULT) (((IDX) < 0 || (IDX) >= NUM_ELEMENTS(ARRAY))?(DEFAULT) : (ARRAY)[IDX])
    ...
    { \
      if(!(e)) { \
        ...
        _assertx_msg << "ASSERTION: " __FILE__ ":" << std::dec << __LINE__ << ": " #e << "==0" << msg << std::endl; \
        MessageBox(/*window*/NULL, /*text*/_assertx_msg.str().c_str(), /*title*/"problem", /*type*/MB_OK); \
      } \
    }
    ...
    { \
      if(__builtin_expect(!(e), 0)) { \
        std::cerr << "ASSERTION: " __FILE__ ":" << std::dec << __LINE__ << ": " #e << "==0" << msg << std::endl; \
        BRK \
      } \
    }
  #endif
...
inline int RndToInt(double v) { return (int)floor(v+0.5); }
template <class T> inline T Rnd(float v) { return (T)(v+0.5f); }
template <class T> inline T Rnd(double v) { return (T)(v+0.5f); }

//-------------------------------------------------------------------------------------------------
//int sprintNum(char* s, float v, float min_unit=0.0f, float max_unit=1e10f);

//-------------------------------------------------------------------------------------------------
template <class T> struct BuiltinWrapper
// wrapper for a built in, handy for deriving from
{
  ...
  BuiltinWrapper& operator = (const T& val_) { val = val_; return *this; }

  bool/*true=val changed, false=val the same*/chkChg(const T& val_)
  {
    if(val == val_) return false;
    val = val_;
    return true;
  }
};
...
//------------------------------------------------------------------------------------------
  ...
  #define SCOPE_NO_FP_EXCEPTIONS_OR_DENORMALS ScopeNoFpExceptionsOrDenormals _scope_no_exceptions_or_denormals_
#endif
...
template <class T>
  inline float lin_interp(float frac, const T& limit/*expect 2 element array*/)
...
//------------------------------------------------------------------------------------------
inline float /* > 0 */ exp_interp(float frac /* 0..1 */, float out_min /* > 0 */, float out_max /* > 0 */)
...
// note that the output can never be quite 0
{
  return expf(
    lin_interp(
      frac,
      logf(std::max(out_min, 1e-36f)),
      logf(std::max(out_max, 1e-36f))
    )
  );
}
...
template <class T>
  inline float exp_interp(float frac, const T& limit/*expect 2 element array*/)
...
//------------------------------------------------------------------------------------------
// vector mutliply src_vec by src_scalar, note that number of elements to process is taken from "src_vec"
template <class DST, class SRC> inline void vec_mult(DST& dst_vec, const SRC& src_vec, typename SRC::Type src_scalar)
  { for(int i = 0; i < src_vec.size(); i++) dst_vec[i] = src_vec[i]*src_scalar; }
...
//------------------------------------------------------------------------------------------
template <class T> inline VecPrintStruct<typename T::Type> vec_print(const T& src, std::string separator)
{
  ...
  // a temp object that goes out of scope by the time we get to print
    ...
    for(int i = 0; i < src.size(); i++) v.data[i] = src[i];
    return v;
}
...
template <class T> inline std::ostream& operator << (std::ostream& o, const VecPrintStruct<T>& v)
{
    ...
    for(int i = 0; i < last; i++) o << v.data[i] << v.separator;
    if(last >= 0) o << v.data[last];
    return o;
}
//------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------
template <class T>
  inline bool idx_within(int idx, const T& t)
...
template <class T> T* begin(const std::vector<T>& v) { return &(const_cast<std::vector<T>&>(v)[0]); }
...
//
{
  ...
  T* Steal() { T* r = ptr; ptr = NULL; return r; }
  ...
  _PtrBase& ForceAssign(void* ptr_) { ptr = (T*)ptr_; return *this; }
};

//------------------------------------------------------------------------------------------
template<class T> struct _Ptr : public _PtrBase<T>
//
...
//
{
  ...
  _Ptr& New() { _PtrBase<T>::ptr = new T; return *this; }

  // delete and clear pointer
  void Delete() { delete _PtrBase<T>::ptr; _PtrBase<T>::ptr = NULL; }

  // new & delete arrays (when calling, careful not to mix with New()/Delete() above)
  void NewArray(unsigned i) { _PtrBase<T>::ptr = new T[i]; }
  void DeleteArray() { delete [] _PtrBase<T>::ptr; _PtrBase<T>::ptr = NULL; }

  // need to define these ones
  _Ptr& operator ++() { _PtrBase<T>::ptr++; return this; } // prefix
  _Ptr& operator --() { _PtrBase<T>::ptr--; return this; } // prefix
...
//------------------------------------------------------------------------------------------
template<class T> class Rng : public _PtrBase<T>
...
public:
  ...
  Rng() { n = 0; }
  Rng(const Null&) { base::ptr = NULL; n = 0; }
  Rng(T* ptr_, int n_ = 1) { base::ptr = ptr_; n = n_; }
  ...
  void setLenFromEnd(int len) { base::ptr = base::ptr + n - len; n = len; }
...
template <class T> inline Rng<T> toRng(Rng<T>& r) { return Rng<T>(r.ptr, r.n); }
template <class T> inline Rng<T> toRng(T* ptr, int n = 1) { return Rng<T>(ptr, n); }
template <class T> inline Rng<T> toRng(std::vector<T>& v) { return Rng<T>(&v[0], (int)v.size()); }
template <class T> inline Rng<T> toRng(std::valarray<T>& v) { return Rng<T>(&v[0], (int)v.size()); }
// return true if the 0 <= idx < rng.size()
template <class T> bool withinRng(int idx, const T& rng) { return idx >= 0 && idx < toRng(const_cast<T&>(rng)).size(); }

// turn an array into a range
#define TO_RNG(c_array) toRng(c_array, NUM_ELEMENTS(c_array))

//-------------------------------------------------------------------------------------------------
struct UCharRng : public Rng<unsigned char>
// Anything that can be cast to a Rng<> can also be cast to an unsigned char range
{
  ...
  template <class T> UCharRng(T* src, int n_items = 1) : base((unsigned char*)src, sizeof(T)*n_items) {}
    
  UCharRng(void* src, int n_bytes) : base((unsigned char*)src, n_bytes) {}
};
...
template <class T> inline UCharRng::UCharRng(T& src) :
  Rng<unsigned char>(toRng(src).toUCharRng()) {}

// memcpy of ranges
inline void CopyFillDst(UCharRng dst, const VoidPtr& src) { dst.copyFrom(src); }
inline void CopyAllSrc(VoidPtr dst, UCharRng src) { src.copyTo(dst); }

// set all bytes to "value"
inline void SetBytes(UCharRng dst, unsigned char value) { memset(dst.ptr, value, dst.n); }

// do a bytewise clear
template <class T> inline void Clear(T* dst, int n) { memset(dst, 0, n*sizeof(T)); }
inline void Clear(UCharRng dst) { SetBytes(dst, 0); }

// array copy
template <class T> inline void Copy(T* dst, const T* src, int n_items) { memcpy(dst, src, n_items*sizeof(T)); }

// set all values in something that can be converted to a range
template <class A, class B> inline void SetAllInRng_(Rng<A> rng, const B& value) { for(int i = 0; i < rng.n; i++) rng[i] = value; }
template <class A, class B> inline void SetAllInRng(A& rng, const B& value) { SetAllInRng_(toRng(rng), value); }
...
//-------------------------------------------------------------------------------------------------
struct LittleEndianMemStr : public UCharRng
// 
// Access a buffer containing data in little endian format
// data is shifted to the start of the buffer
//
{
  template <class T> LittleEndianMemStr(T& t) : UCharRng(t) {}
  LittleEndianMemStr(unsigned char* mem, int n_bytes) : UCharRng(mem, n_bytes) {}
  explicit LittleEndianMemStr(void* mem, int n_bytes) : UCharRng(mem, n_bytes) {}
...
public:
  ...
  void incRefCount() const { if(_ref_count >= 0) InterlockedIncrement(&_ref_count); }
...
//-------------------------------------------------------------------------------------------------
template <class T> class Ref : public _PtrBase<T>
//
// reference counting template
//
{
public:
  Ref(T *x = 0) { _PtrBase<T>::ptr = 0; set(x); }
  Ref(const Ref& src) { _PtrBase<T>::ptr = 0; set(src); }
  ~Ref() { set(0); }
  ...
  void set(T* new_ptr)
  {
    ...
    if(new_ptr) new_ptr->incRefCount();
    if(old_ptr && !old_ptr->decRefCount()) delete old_ptr;
  }
  ...
  Ref& operator = (T* src) { set(src); return *this; }
...
template <class T, int N> inline Rng<T> toRng(Array<T, N>& src) { return src.rng(); }
...
template <int LENGTH>
  class CharArray : public Array<char, LENGTH> {
public:
  typedef Array<char, LENGTH> base;

  // ensure NULL termination
  void terminate() { if(LENGTH > 0) base::data[LENGTH-1] = 0; }
  ...
  CharArray(Rng<char> src /*0 terminated*/)
  {
    ...
    char* dst = base::data;
    while(n > 0 && *src.ptr) { *dst++ = *src.ptr++; n--; }
  ...
  CharArray(const char* src /*0 terminated*/)
  {
    ...
    if(src) {
      ...
      while(i < LENGTH-1 && *src) base::data[i++] = *src++;
    }
  ...
  void strcat(const char* src) { strncat(base::data, src, LENGTH); terminate(); }
  ...
  Rng<char> unused() { int n = strlen(); return toRng(base::data+n, LENGTH-n); }
...
template <int N> inline Rng<char> toRng(CharArray<N>& src) { return src.rng(); }

template <int LENGTH> bool operator == (const CharArray<LENGTH>& a, const char* b) { return strncmp(a, b, LENGTH) == 0; }
template <int LENGTH> bool operator == (const char* a, const CharArray<LENGTH>& b) { return strncmp(a, b, LENGTH) == 0; }
template <int LEN1, int LEN2> bool operator == (const CharArray<LEN1>& a, const CharArray<LEN2>& b) { return strncmp(a, b, std::min(LEN1,LEN2)) == 0; }

//------------------------------------------------------------------------------------------
template <int LENGTH>
  struct SprfCharArray : public CharArray<LENGTH>
{
...
//------------------------------------------------------------------------------------------
template <class VAL, class STP=VAL> struct ValStp : public BuiltinWrapper<VAL>
// structure to hold a value and a step
{
  ...
  STP stp;
  ValStp(VAL val_ = 0, STP stp_ = 0) { base::val = val_; stp = stp_; }
  ValStp& operator = (const VAL& val_) { base::val = val_; }

  ValStp& next(STP i) { base::val += stp*i; return *this; }
  ValStp& prev(STP i) { base::val -= stp*i; return *this; }
  ValStp& next() { base::val += stp; return *this; }
  ValStp& prev() { base::val -= stp; return *this; }

  ValStp& set(VAL val_, STP stp_) { base::val = val_; stp = stp_; return *this; }
...
template <class T> struct FindMinMax : public Array<T, 2> 
{
  ...
  FindMinMax(
    T min_v,	// choose something big initially
  ...
  void operator () (T v)
  {
    if(v < base::data[0]) base::data[0] = v;
    if(v > base::data[1]) base::data[1] = v;
  }
...
//*************************************************************************************************
template <int _N> struct SplitParam
//
...
//
{
  ...
  // set parts
  void operator () (int i, float f) { i_part = i; f_part = f; }
...
template <class PTR> inline bool isOk(PTR ptr) { return ptr && ptr->ok(); }


//*************************************************************************************************
template <class A, class B> inline bool/*true=was different, false=was same*/
...
//-------------------------------------------------------------------------------------------------
struct ChkChg : public BuiltinWrapper<bool>
// wrapper to record whether a value was changed
{
  ...
  // constructor
  template <class A, class B> ChkChg(A& a, const B& b) { *this = false; operator()(a, b); }
...
template <class DST> template <class SRC> inline bool ChkChgPtr<DST>::operator= (const SRC& src) { return (*chg)(*ptr, src); };

//-------------------------------------------------------------------------------------------------
template <class T> struct ValueChange
//
...
//
{
  ...
  ValueChange(const T& initial_val, bool initial_chg) { val = initial_val; chg = initial_chg; }
  ...
  // assignment works
  ValueChange& operator = (const T& v) { set(v); return *this; }

  // force set
  void set(const T& v, bool force_chg) { val = v; chg = force_chg; }
...
//
{
  ...
  // see "find" below
  template <class ARRAY> FindClosestArrayElement(
    ARRAY& values, float/*0..1*/approx_idxf, float approx_value, float aspect_ratio
  )
  {
    init();
  ...
  template <class ARRAY> int find(
    ARRAY& values,					// array of values to search over
  ...
  )
  {
    ...
    if(n > 1) idxf.stp = aspect_ratio/((float)n-1);
...
// rescale a frac (0..1) value to a new range with offset
{
  ...
  float/*pixel*/getRescaled(float/*0..1*/frac) const
    { return frac*eff_len + offset; }
...
//-------------------------------------------------------------------------------------------------
struct CharRng : public Rng<char>
//
...
//
{
  CharRng(Rng<char> src) : Rng<char>(src) {}
...
inline CharRng operator << (CharRng dst, const char* s /*zero terminated*/)
{
  ...
  // copy string
  while(dst.n > 0 && *s) { *dst.ptr++ = *s++; dst.n--; }
...
inline CharRng operator << (CharRng dst, Rng<const char> src /*zero terminated*/)
{
  ...
  if(ChkMemOverlap(dst, src)) src.n = strnlen(src, src.n);

  // copy string
  int n = std::min(dst.n-1, src.n), n_start = n;
  while(n > 0 && *src.ptr) { *dst.ptr++ = *src.ptr++; n--; }
...
//
inline CharRng operator << (CharRng dst, const RngPrintNumStruct& s) { return s.print(dst); }
...
//
inline CharRng operator << (CharRng dst, int n) { return dst << sprf("%d", n); }
inline CharRng operator << (CharRng dst, unsigned int n) { return dst << sprf("%u", n); }
inline CharRng operator << (CharRng dst, float n) { return dst << sprf("%f", n); }
inline CharRng operator << (CharRng dst, double n) { return dst << sprf("%lf", n); }
...
//   // str contains now "50%"
//
struct PercentStruct { float frac; int decimal_places; };

inline PercentStruct spr_percent(float /*0..1*/ frac, int decimal_places=0)
  { PercentStruct r = { frac, decimal_places }; return r; }
