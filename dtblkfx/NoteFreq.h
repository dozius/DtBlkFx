...
inline float NoteOffsToHz(float note_offs_c0) { return NoteFreq::c0_hz*powf(2.0f, note_offs_c0*(1.0f/12.0f)); }
...
//
struct NoteToTextStruct { float offs_c0; };
inline NoteToTextStruct NoteToTxt(float offs_c0) { NoteToTextStruct r = { offs_c0 }; return r; }
inline NoteToTextStruct HzToNote(float freq) { NoteToTextStruct r = { HzToNoteOffs(freq) }; return r; }
inline CharRng operator << (CharRng dst, NoteToTextStruct s) { return NoteToTxt(dst, s.offs_c0); }
