...
inline int PackedBytesPerVstProgram(int n_params) { return 32+4*n_params; }
...
public: // name access
	...
	//
	void clear() { Clear(name); Clear(params); }
...
public: // various ways of loading
	void loadFromString(
		const char* str			// string containing contains "<name>:<param> <param> ..."
	)
	{
		clear();
		...
		while(*str) {
			...
			if(idx_within(i, name)) name[i++] = *str;
			str++;
		}
		name[i] = '\0';
		...
		for(i = 0; i < N_PARAMS; i++) {
			...
			if(/*end-of-string*/ next_str == str) break;
			str = next_str;
		}
	}
	
	bool loadLittleEndian(
		LittleEndianMemStr* src_data,	// data to load from
	...
	{	
		...
		if(!ShiftSrc(name, src_data)) return false;
		name.terminate();

		// copy floats
		int i;
		for(i = 0; i < num_params && i < N_PARAMS; i++)
			if(!src_data->get32(params+i)) return false;

		// read dummy values
		float dummy;
		for(; i < num_params; i++)
			if(!src_data->get32(&dummy)) return false;

		return true;
	}

	bool saveLittleEndian(
		LittleEndianMemStr* dst_data,	// data to load from
	...
	)
	{
		...
		int i;
		for(i = 0; i < num_params && i < N_PARAMS; i++)
			if(!dst_data->put32(params[i])) return false;
			
		for(; i < num_params; i++)
			if(!dst_data->put32(0.0f)) return false;
