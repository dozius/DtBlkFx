...
//-------------------------------------------------------------------------------------------------
Array<const char*, 7> g_sgram_menu_strings =
	{ "Input L+R", "Input Left", "Input Right", "Output L+R", "Output Left", "Output Right", "Paused" };
...
//-------------------------------------------------------------------------------------------------
Gui::Gui (AudioEffect *effect) : AEffGUIEditor (effect)
{
	...
	rect.bottom = (VstInt16)(
		Images::g_glob_bg->getHeight()+
		Images::g_splash_bg->getHeight()+ // spectrograms
...
bool/*success*/Gui::openSgram(int i, CPoint* p)
{
	...
	CRect splash_bg_rect = GetTile(Images::g_splash_bg, /*num x*/1, /*num y*/2, /*idx x*/0, /*idx y*/i);
	...
	//
	_sgram[i]->init(
		MoveTo(*p, splash_bg_rect),	// size of window
...
//
{
	...
	if(!_glob_ctrl->ok()) return false;
	p.y += Images::g_glob_bg->getHeight();

	// place spectrograms
	for(i = 0; i < 2; i++)
		if(!openSgram(i, &p)) return false;
...
//-------------------------------------------------------------------------------------------------
void Gui::FFTDataRdy(
	int a			// 0=input data, 1=output data
)
// preconditions:
//   DtBlkFx::_protection is locked
{
	...
	for(int sgram_i = 0; sgram_i < 2; sgram_i++) {
		if(!_sgram[sgram_i]->rdy()) continue;
		...
		// for safety
		if(!withinRng(menu_val, g_sgram_menu_strings)) continue;
		...
		for(int ch = 0; ch < 2; ch++) {
			if(g_stereo_ch_menu_map[a][menu_val][ch]) {
				...
				if(a == 1) pwr_scale = blkFx()->_chan[ch].out_pwr_scale;
				_sgram[sgram_i]->newData(_pix_bin[ch].getMap(plan), pwr_scale);
				update = true;
			}
		}

		if(update) _sgram[sgram_i]->blkDone(samp_pos, time_fft_n);
	}
}
...
//   DtBlkFx::_protection is locked
{
	...
	for(int i = 0; i < 2; i++) _sgram[i]->suspend();
}
