...
//
{
	...
	if(!v->isDirty()) return update_rect.rectOverlap(GetRect(v));

	// otherwise view is dirty

	//only clear dirty flag if it can be completely redrawn
	if(IsContained(update_rect, v)) v->setDirty(false);
...
// alpha blend function using VST gui's function pointer to alphablend
{
	...
	extern BOOL (WINAPI *pfnAlphaBlend)(
	...
	return (*pfnAlphaBlend)(
		(HDC)dst_context->getSystemContext(), // handle to destination DC
...
CContextRGBA::CContextRGBA (
	int width,		// pixels (default 0)
	int height		// pixels (default 0)
) :
	CDrawContext (
		NULL,		// frame - don't need it we're always RGBA
		NULL,		// system context
		NULL		// window
	)
{
	...
	if(width && height) create(width, height);
}
...
// expect pSystemContext to have been filled in
{
	...
	void *data_ = NULL;
	_hbm = CreateDIBSection((HDC)pSystemContext, (BITMAPINFO*)&bm_info, DIB_RGB_COLORS, &data_, NULL, 0x0);
	if(!_hbm) return;
...
//
{
	...
	// create by copying from <resource_bitmap_id>
	{
		...
		if(!bm_params.bmWidth || !bm_params.bmHeight) return;
...
//-------------------------------------------------------------------------------------------------
void CContextRGBA::createFromResourceBitmap(
	int resoure_bitmap_id	// resource bitmap to copy
)
{
...
// RGBA as 32 bit host-order colour
{
	...
	for(int i = 0; i < dst.n; i++) {
		...
		if(i >= end_i) {
			...
			if(col_vec_i < col_vec.n-1) col_vec_i++;
			...
			if(end_i < beg_i+1) end_i = beg_i+1;
		...
		for(int j = 0; j < 4; j++)
			c[j] = (unsigned)limit_range(
				lin_interp(frac, a[j+1], b[j+1])*255.0f,
				0.0f, 255.0f
			);
...
//-------------------------------------------------------------------------------------------------
DtHSlider::DtHSlider() : CControl(CRect())
{
	init();
}
DtHSlider::~DtHSlider() {}
...
// virtual, override CView
{
	...
	setState(
		isInKnob(/*parent coords*/mouse) ? DRAGGING : STEPPING,
		mouse.x,	// click pos x
...
// sets the value & control to dirty if the value changed
{
	...
	if (send_to_listener && listener) listener->valueChanged (this);
}
...
//-------------------------------------------------------------------------------------------------
void DtPopupWdw::update(
	CastCRect sz,					// screen position & size of window
...
//-------------------------------------------------------------------------------------------------
void DtPopupWdw::move(
	CPoint top_left			// top-left screen position
)
{
...
//-------------------------------------------------------------------------------------------------
void DtPopupHSliderInternal::create(
	CFrame* frame,	//
...
//-------------------------------------------------------------------------------------------------
void DtPopupHSliderInternal::shift(
...
//-------------------------------------------------------------------------------------------------
DtPopupHSlider::DtPopupHSlider() : COptionMenu(CRect(), /*listener*/NULL, /*tag*/-1)
{
	//
	setMenuSnap(false);
...
void DtPopupHSlider::setMenuSnap(bool snap_state)
{
	...
	if(snap_state) setStyle(getStyle() | kCheckStyle);
	else setStyle(getStyle() & ~kCheckStyle);
}
...
// is called by "COptionMenu::takFocus()"
{
	...
	// send the new value to the listener if need be (due to GUI activity)
	if (send_to_listener && _listener) _listener->valueChanged (this);
}
...
//-------------------------------------------------------------------------------------------------
int DtPopupHSlider::addMenuEntry(
	float value,		// 0..1 or -1 for no associated value
...
//-------------------------------------------------------------------------------------------------
bool DtPopupHSlider::setMenuEntry(
	long index,			// menu index
	float value,		// 0..1 or -ve for no associated value
	const char* txt		//
)
{
	if(!idx_within(index, _menu_item_to_value)) return false;

	if(!COptionMenu::setEntry(index, const_cast<char*>(txt))) return false;
...
{
	// for safety
	if(!popupsAreOpen()) return;
	if(_state < PRE_DRAG) return;
	...
	// shift other popup
	if(_state > PRE_DRAG)
		_slider[_state==COARSE ? FINE : COARSE].shift(
			v,
			_slider[_state].getPixel(v)
		);
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult DtPopupHSlider::onMouseUp (CPoint &mouse, const long& buttons)		///< called when a mouse up event occurs
// virtual, override CView
{
	// make sure popups are closed
	for(int i = 0; i < 2; i++) _slider[i].close();

	// release edit
	if(_state > PRE_DRAG) endEdit();
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult DtPopupHSlider::onMouseMoved (CPoint &mouse, const long& buttons)	///< called when a mouse move event occurs
// virtual, override CView
{
	ChkChg state_chg;
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult DtPopupHSlider::onMouseEntered (CPoint &mouse, const long& buttons)	///< called when the mouse enters this view
// virtual, override CView
{
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult DtPopupHSlider::onMouseExited (CPoint &mouse, const long& buttons)	///< called when the mouse leaves this view
// virtual, override CView
{	
	_state = INACTIVE;
	setDirty();
...
//-------------------------------------------------------------------------------------------------
bool DtPopupHSlider::onWheel (const CPoint &where, const CMouseWheelAxis &axis, const float &distance, const long &buttons)	///< called if a mouse wheel event is happening over this view
// virtual, override CView
{
	float delta = CControl::wheelInc * distance * (buttons&kControl ? _fine_scale_frac : 1.0f);
	beginEdit();
...
// called when there's a mouse click to determine whether we should take the click or not
{
	...
	if((buttons&_hittest_button_mask) == 0) return false;
...
//
{
	...
	// of range
	if(!within(v, 0.0f, 1.0f)) v = _value_before_take_focus;
...
// open our popup sliders
{
	...
	if(_state >= MENU_OPEN) return kMouseEventHandled;
	...
	for(i = 0; i < 2; i++) {
		...
		_slider[i].shift(
			v,
			(float)(mouse.x+_wdw_screen_offs.x)
		);
	}
...
DtOwnerToggle::DtOwnerToggle() : CControl(CRect())
{
	init();
}
...
{
	// do nothing if 
	if(new_value == getBoolValue()) return;
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult DtOwnerToggle::onMouseDown (CPoint &mouse, const long& buttons)		///< called when a mouse down event occurs
// virtual, override CView
{
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult DtOwnerToggle::onMouseUp (CPoint &mouse, const long& buttons)		///< called when a mouse up event occurs
// virtual, override CView
{
	// don't do anything if we didn't get the mouse press
	if(_state <= FOCUS) return kMouseEventHandled;
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult DtOwnerToggle::onMouseMoved (CPoint &mouse, const long& buttons)	///< called when a mouse move event occurs
// virtual, override CView
{
	ChkChg state_chg;
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult DtOwnerToggle::onMouseEntered (CPoint &mouse, const long& buttons)	///< called when the mouse enters this view
// virtual, override CView
{
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult DtOwnerToggle::onMouseExited (CPoint &mouse, const long& buttons)	///< called when the mouse leaves this view
// virtual, override CView
{
	_state = INACTIVE;
	setDirty();
...
//-------------------------------------------------------------------------------------------------
COptionMenuOwnerDraw::COptionMenuOwnerDraw() : COptionMenu(CRect(), /*listener*/NULL, /*tag*/-1)
{
	init();
...
//-------------------------------------------------------------------------------------------------
CMouseEventResult COptionMenuOwnerDraw::onMouseDown (CPoint &mouse, const long& buttons)		///< called when a mouse down event occurs
{
	_state = INACTIVE;
	setDirty();
	return COptionMenu::onMouseDown(mouse, buttons);
