...
//-------------------------------------------------------------------------------------------------
void PixelFreqBin::init(
	Rng<float> pixel_to_hz,	// Hz for each pixel (and number of pixels)
	float sample_freq,		//
...
)
{
	...
	for(int i = 0; i < NUM_FFT_SZ; i++) {
		...
		for(x = 1; x < pixel_to_hz.size(); x++) {
			int bin = limit_range(
				RndToInt(pixel_to_hz[x]*hz_to_bin),
				prev_bin, max_bin // gaurantee increasing
			);
			map[i][x] = fft_dat+bin;
			prev_bin = bin;
		}
