...
protected:
	...
	// current interpolation fraction of between _out_a & _out_b (set by setOutPos) 0.0f=_out_a, 1.0f=_out_b
	float _interp_frac;

	// increment index "i", wrapping to 0 if need be
	void incIndex(int &i) { if(++i>=_length) i = 0; }
...
protected:
	...
	{
		return _use_override_param[param_idx] && !allow_override ? _override_param[param_idx] : vals(row)[param_idx];
	}
...
public:
	...
	void init(int n_params, int length)
	{
		...
		_field_byte_offs[0] = sizeof(long)+sizeof(float)*_n_params;	// any_explicit_set array: skip samp_abs_new field & vals array
		_field_byte_offs[1] = _field_byte_offs[0]+sizeof(bool);			// explicit_set array: skip any_explicit_set
		_record_len_bytes = _field_byte_offs[1]+sizeof(bool)*_n_params;	// total length: skip explicit_set
...
public: // parameter override (use for param preview)
	...
	void setOverride(VstParamIdx idx, float v)  { setOverride(idx, true); _override_param[idx] = v; }
...
public:
	...
	float getInterp(VstParamIdx idx) const { return lin_interp(_interp_frac, getPrev(idx), getNext(idx)); }

	// get the absolute sample position for the current param or next out param
	long getOutSampAbs(bool next_param=false) const { return samp_abs(next_param?_out_b:_out_a); }
	...
	bool isExplicitlySet(VstParamIdx idx, bool next_param=false) const { return explicit_set(next_param?_out_b:_out_a)[idx]; }

	// return true if any param is explicitly set
	long isAnyExplicitSet(bool next_param=false) const { return any_explicit_set(next_param?_out_b:_out_a); }
	...
	// reset the params vector and copy the most recent params but reset abs time to 0
	{
		...
		memcpy(data<char>(/*idx*/0, /*byte offs*/0), data<char>(/*idx*/_in, /*byte offs*/0), _record_len_bytes);
	...
	// return true if the param already exists
	{
		...
		if(samp_diff <= 0) already_exists = true;
		else {
...
protected:
	...
	//
	{
		...
		if(next == _out_b) incIndex(_out_b);
...
//-------------------------------------------------------------------------------------------------
struct ParamsDelayGet : public _PtrBase<const ParamsDelay>
...
//
{
	...
	Fn fn;
	ParamsDelayGet(const ParamsDelay& p, Fn f) { ptr = &p; fn = f; }
...
//
inline ParamsDelayGet GetInput(const ParamsDelay& p) { return ParamsDelayGet(p, &ParamsDelay::getInput); }
inline ParamsDelayGet GetInputNoForced(const ParamsDelay& p) { return ParamsDelayGet(p, &ParamsDelay::getInputNoForced); }
inline ParamsDelayGet GetPrev(const ParamsDelay& p) { return ParamsDelayGet(p, &ParamsDelay::getPrev); }
inline ParamsDelayGet GetNext(const ParamsDelay& p) { return ParamsDelayGet(p, &ParamsDelay::getNext); }
inline ParamsDelayGet GetInterp(const ParamsDelay& p) { return ParamsDelayGet(p, &ParamsDelay::getInterp); }
