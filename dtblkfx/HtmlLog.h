...
public:
	...
	std::ostream& startline(
		const char* style		// eg: "color:#ff0000;background:#000000" for red on black
	)
	{
		...
		if(print_style) _f << "<span style='" << style << "'>";
		else _f << "<span>";
	...
	std::ostream& startline(
		unsigned long fg_bg_12bit	// eg 0xF00000 for red on black
	)
	{
		return startline(
			SprfCharArray<32>("color:#%03x;background:#%03x", (fg_bg_12bit>>12)&0xFFF, fg_bg_12bit&0xFFF)
		);
	}
...
#define HTML_LOG(html_log, style, stream) \
	{\
		try { (html_log).startline(style) << stream; } catch(...) {} \
		(html_log).endline(); \
	}
...
#define DBG_HTML_LOG(html_log, html_style, stream) \
	{\
		...
		(html_log).startline(style) << _html_log_stream_.str(); (html_log).endline(); \
...
public:
	...
	)
	{
		...
		_line << "<tr>";
		_line << "<th>time";
		for(int i = 0; i < n_labels; i++) _line << "<th>" << labels[i];
		_hdr = _line.str();
		_line.str("");
		_line_n = 0;
		_lines_per_pg = 30;
	...
	void flushLine()
	{
		...
		_f << "<tr><td>"
			<< SprfCharArray<16>("%.3f", (float)clock()/1000.0f)
			<< _line.str()
			<< std::endl;

		_line.str("");
		if(++_line_n >= _lines_per_pg) _line_n = 0;
	}
	...
	HtmlTable& operator << (Dummy& dummy) { flushLine(); return *this; }
};
