...
//-------------------------------------------------------------------------------------------------
Spectrogram::Spectrogram() : CViewContainer(CRect(), /*CFrame* parent*/NULL)
{
...
//
{
	...
	if(in > _pwr_min) {
		...
		if(out >= _col_map.n) out = _col_map.n-1;
	}
	else out = 0;
...
//------------------------------------------------------------------------------------------
void Spectrogram::newData(
	cplxf** bin_rng,	// appropriate mapping from PixelFreqBin
...
)
{
	...
	if(samp_pos+time_fft_n-_curr_line_samp_pos < /*Spectrogram::*/min_samps_per_line) return;
	...
	// circular buffer
	_image_i++;
	if(_image_i >= _image.getHeight()) _image_i = 0;
...
// use _disp_i[0] as the index of most recent line in image buffer
{
	...
	while(1) {
		...
		if(src_i < 0) {
			...
			if(max_lines < n_lines) n_lines = max_lines;
			temp = true;
		}
...
// and new rects (_hilite_x[1])
{
	...
	if(_hilite_x[0] == _hilite_x[1]) return false;
	...
	while(curr_x < max_x) {
		...
		if(do_invert) {
			...
			if(r.width() > 0) InvertRect(context, r);
		}
	...
	if(_info_draw_state) _info_draw_state = OVERLAY_NEEDS_UPDATE;
	if(_name_draw_state) _name_draw_state = OVERLAY_NEEDS_UPDATE;
...
// must be called periodically, performs scrolling if need be and displays mouse information
{
	...
	// note: _disp_i[1] may be updated asynchronously to this function - ensure code is multi-thread safe
	int scroll_n = new_disp_i-_disp_i[0];
	if(scroll_n < 0) scroll_n += _image.getHeight();
	...
	if(_info_draw_state) drawBm(context, info_rect);
	if(_name_draw_state) drawBm(context, name_rect);
	_info_draw_state = OVERLAY_NOT_DRAWN;
...
// virtual override CViewContainer::drawRect
{
	...
	if(_info_draw_state == OVERLAY_DRAWN) _info_draw_state = OVERLAY_NEEDS_UPDATE;
	if(_name_draw_state == OVERLAY_DRAWN) _name_draw_state = OVERLAY_NEEDS_UPDATE;
...
// virtual, override CViewContainer
{
	...
	if(r != kMouseEventNotHandled) return r;
...
{
	if (!pParentFrame) return NULL;
	if(!IsInside(_display_rect, p)) return NULL;
