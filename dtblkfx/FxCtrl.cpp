...
inline DtBlkFx* FxCtrl::blkFx() { return _gui->blkFx(); }


//-------------------------------------------------------------------------------------------------
FxCtrl::FxCtrl() :
	CViewContainer(
		CRect(0,0,0,0),		//const CRect &rect,
		NULL,				//CFrame *pParent,
...
{
	ctrl->setListener(fx);
	ctrl->interceptMouseEvents(/*target*/fx, /*pre handle*/&FxCtrl::mouseInterceptBefore, /*post handle*/&FxCtrl::mouseInterceptAfter);
	AddView(ctrl, fx);
}
...
//
{
	...
	for (i = 0; i < g_num_fx_1_0; i++) {
		...
		if(strcmp(fx->name(), "DoNotUse") == 0) continue;
...
{
	if(_freq[0]->getState()!=_freq[0]->INACTIVE) return _freq[0];
	if(_freq[1]->getState()!=_freq[1]->INACTIVE) return _freq[1];
	if(_amp->getState()!=_amp->INACTIVE) return _amp;
	if(_fxtype_ctrl->getState() != _fxtype_ctrl->INACTIVE) return _fxtype_ctrl;
	if(_fxval->getState() != _fxval->INACTIVE) return _fxval;
	return NULL;	
}
...
// virtual, override CViewContainer
{
	...
	if(fx_state == MASK_EFFECT) {
		if(!idx_within(_fx_set+1, blkFx()->_fx1_0)) {
			...
			if(next_fx->isMask()) fx_state = MASK_NO_EFFECT;
		}
	}
	...
	if(fx_state != MASK_NO_EFFECT && (active_ctrl || _drag_both_state)) {
		...
		if(_drag_both_state) active_ctrl_r = EnclosingRect(_freq[0], _freq[1]);
		else active_ctrl_r = active_ctrl;
		...
		// blend
		for(i = 0; i < 3; i++)
			Images::g_fx_bg->blend(/*dest*/temp_bm, dst_r[i], src_p[i]+TopLeft(dst_r[i]), /*alpha*/255, /*use src alpha*/false);
	}
	else {
		// row not selected
		Images::g_fx_bg->blend(/*dest*/temp_bm, /*dest*/CPoint(0,0), /*src rect*/bg_inactive_r, /*alpha*/255, /*use src alpha*/false);
	}

	// for mask effects, remove the 1 pixel high separating line at the bottom by copying the second last line of the
	// inactive rectangle
	if(fx_state == MASK_EFFECT) {
		// bottom line
	...
	if(!fft_fx->paramUsed(BlkFxParam::FX_AMP)) amp_alpha = 63;
	else if(_amp->getState() == _amp->INACTIVE) amp_alpha = 127;
	Images::g_fx_bg->blend(/*dst contect*/temp_bm, /*dest*/CPoint(amp_r.left, 0), /*src*/amp_r, /*alpha*/amp_alpha);
	...
	for(i = 0; i < 2; i++) {
		...
		freq_pos[i] = RndToInt(lin_interp(freq_val[i], (float)freq_scale_r.left+5, (float)freq_scale_r.right-5));
	}

	unsigned char freq_alpha = 
		fft_fx->paramUsed(BlkFxParam::FX_FREQ_A) || fft_fx->paramUsed(BlkFxParam::FX_FREQ_B) ? 255 : 127;
	...
	if(freq_val[0] <= freq_val[1]) {
		...
		r.left = freq_pos[0];
		r.right = freq_pos[1];
		Images::g_fx_bg->blend(/*dst*/temp_bm, /*dst*/CPoint(r.left, 0), /*src rect*/r, /*alpha*/freq_alpha);
	}
	else {
	...
	// effect type text
	temp_bm->setFontColor(_fxtype_ctrl->getState()==_fxtype_ctrl->INACTIVE?normal_txt_col:focus_txt_col);
	...
	temp_bm->blend(&dc, /*dst rect*/this, /*src point*/CPoint(0,0), /*overall alpha*/255, /*src alpha*/false);
}
...
void FxCtrl::doHiliteSgrams()
{
	...
	//
	if(!active_ctrl && !_drag_both_state) return;
...
void FxCtrl::updateFxValMenu()
{
	...
	if(fft_fx == _fxval_menu_type) return;
...
void FxCtrl::updateAmpMenu()
{
	...
	if(amp_menu_mixmode == _amp_menu_mixmode) return;
...
void FxCtrl::mouseInterceptBefore(Event::Args* args)
{
	...
	if(mouse->event_type == Event::kMouseDown && mouse->buttons&kRButton) {
		if(mouse->src_view == _amp) updateAmpMenu();
		else if(mouse->src_view == _fxval) updateFxValMenu();
	}
...
// called by the gui control when changed by the user
{
	...
	// note that all other controls already give their value 0..1
		...
		if(!idx_within((int)param_val, _map_menu_to_param)) return;
		...
		if(_fx_set > 0) _gui->_fx_ctrl[_fx_set-1]->setDirty();
	}
...
//
{
	...
	switch(index) {
		case BlkFxParam::FX_FREQ_A: _freq[0]->setValue(v); doHiliteSgrams(); break;
		case BlkFxParam::FX_FREQ_B: _freq[1]->setValue(v); doHiliteSgrams(); break;
		case BlkFxParam::FX_AMP: _amp->setValue(v); break;
		case BlkFxParam::FX_TYPE: _fxtype_ctrl->setValue(_map_effect_to_menu[BlkFxParam::getEffectType(v)]); break;
		case BlkFxParam::FX_VAL: _fxval->setValue(v); break;
	}
}
