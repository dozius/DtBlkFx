...
// got images ok?
bool GlobalInitOk() { return g_load_state == GLOBAL_LOAD_STATE_INIT_OK; }
...
g_load_images[] =
{
  ...
  { &DtPopupHSlider::Images::g_knob[DtPopupHSlider::COARSE],	FILE_PREFIX "hslider_knob_coarse.png" },
...
struct LoadPresets
{
  ...
  LoadPresets(ostream* err)
  {
    ...
    CharArray<8192> line;
    while(1) {
      if(!fgets(line, line.size(), f)) break;

      g_blk_fx_presets.push_back(
        VstProgram<BlkFxParam::TOTAL_NUM>(line)
      );
    }
  }
...
VST_EXPORT AEffect *VSTPluginMain(audioMasterCallback audioMaster)
{
  ...
  try {
    ...
    if(g_load_state == GLOBAL_LOAD_STATE_INIT_FAIL) return NULL;
    ...
    if(g_load_state == GLOBAL_LOAD_STATE_NEED_INIT) {
      ...
      if(!LoadPresets(&err_str)) image_error = true;
