...
//------------------------------------------------------------------------------------------
template <class P> inline long wrapProcess(
...
{
	float *ptr, scale;
	P1Src(float* ptr_ = NULL, float scale_ = 1) { ptr = ptr_; scale = scale_; }
	void setLen(long n) {}
...
// mix src into dst
{
	...
	void setMix(float start, float end) { mix.val = start; mix.stp = end-start; }

	void setLen(long n) { mix.stp /= (float)n; }
	void operator() (float* dst, float src) { *dst = *dst * mix + src * (1-mix); mix.next(); }
};
struct PRampDst : public PMixDst
{
	void operator() (float* dst, float src) { *dst = *dst * mix + src; mix.next(); }
};
//------------------------------------------------------------------------------------------
template <class SRC, class DST> struct PSrcDstWrap
// dst is the fifo
{
	SRC src;
	DST dst;
	void setLen(long n) { src.setLen(n); dst.setLen(n); }
	void process(float *dst_data, long n)
	{
		float *dst_e = dst_data+n;
		for(; dst_data != dst_e; dst_data++) dst(dst_data, src());
	}
};
...
//------------------------------------------------------------------------------------------
struct PAddOut : public PCopyOut
...
//------------------------------------------------------------------------------------------
template <class INTERP_PROC, int REVERSE=0> struct PLinInterp
//
//
{
...
public: // internal method
	void prepSeg()
	{
		...
		if(REVERSE) { if(interp_i > 0) interp_i--; }
		else { if(interp_i < interp.n-1) interp_i++; }
