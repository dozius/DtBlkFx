...
struct CColorRGB : public CColor
...
{
	CastCRect() {}
	template <class SRC_RECT> CastCRect(const SRC_RECT& src_rect) : CRect(GetRect(src_rect)) {}
};

//
inline CRect GetRect(const CRect& src) { return CRect(src); }
inline CRect GetRect(const CBitmap* bitmap) { return CRect(0, 0, bitmap->getWidth(), bitmap->getHeight()); }
inline CRect GetRect(const CView* view) { CRect r; view->getViewSize(r); return r; }

// various functions to convert between CRect & CPoint
inline CRect MoveTo(const CPoint& top_left, const CastCRect& r) { return CRect(top_left.x, top_left.y, r.width()+top_left.x, r.height()+top_left.y); }
inline CRect MoveToOrigin(const CastCRect& r) { return MoveTo(CPoint(0,0), r); }
inline CRect Offset(CPoint p, CastCRect r) { return r.offset(p.x, p.y); }
inline CRect MoveLeftEdge(int x, const CastCRect& r) { return CRect(x, r.top, x+r.getWidth(), r.bottom); }
inline CRect MoveTopEdge(int y, const CastCRect& r) { return CRect(r.left, y, r.right, y+r.getHeight()); }
inline CRect MoveRightEdge(int x, const CastCRect& r) { return CRect(x-r.getWidth(), r.top, x, r.bottom); }
inline CRect MoveBottomEdge(int y, const CastCRect& r) { return CRect(r.left, y-r.getHeight(), r.right, y); }

inline CRect Bound(CastCRect r1, const CastCRect& r2) { r1.bound(r2); return r1; }

inline CPoint TopLeft(const CastCRect& r) { return CPoint(r.left, r.top); }
inline CPoint TopRight(const CastCRect& r) { return CPoint(r.right, r.top); }
inline CPoint BottomRight(const CastCRect& r) { return CPoint(r.right, r.bottom); }
inline CPoint BottomLeft(const CastCRect& r) { return CPoint(r.left, r.bottom); }

// get rectangle that encloses r1 & r2
inline CRect EnclosingRect(const CastCRect& r1, const CastCRect& r2) { return CRect(std::min(r1.left, r2.left), std::min(r1.top, r2.top), std::max(r1.right, r2.right), std::max(r1.bottom, r2.bottom)); }
...
inline CRect TileOffset(const CastCRect& rect, int idx_x, int idx_y, int num_tiles_x=1, int num_tiles_y=1)
{
...
inline CRect GetTile(CastCRect r, int div_x, int div_y, int idx_x=0, int idx_y=0, int num_tiles_x=1, int num_tiles_y=1)
{
...
inline CPoint operator - (CPoint a) { return CPoint(-a.x, -a.y); }
inline CPoint operator + (CPoint a, CPoint b) { return CPoint(a.x+b.x, a.y+b.y); }
inline CPoint operator - (CPoint a, CPoint b) { return operator +(a, -b); }
inline CPoint& operator += (CPoint& a, CPoint b) { return a.offset(b.x, b.y); }
inline CPoint& operator -= (CPoint& a, CPoint b) { return operator +=(a, -b); }

// return centre point of rectangle
inline CPoint GetCent(const CastCRect& r) { return CPoint((r.left+r.right)/2, (r.top+r.bottom)/2); }

// return rectangle "r" centered horizontal at cx
inline CRect MoveXCent(int cx, CastCRect r) { int w=r.width(); r.left = cx-w/2; r.right = r.left+w; return r; }
inline CRect MoveXCent(const CastCRect& cx, CastCRect r) { return MoveXCent((cx.left+cx.right)/2, r); }

// return rectangle "r" centered vertical at cy
inline CRect MoveYCent(int cy, CastCRect r) { int h=r.height(); r.top = cy-h/2; r.bottom = r.top+h; return r; }
inline CRect MoveYCent(const CastCRect& cy, CastCRect r) { return MoveXCent((cy.top+cy.bottom)/2, r); }

// return rectangle "r" centred at point "p"
inline CRect MoveCent(CPoint p, const CastCRect& r) { return MoveYCent(p.y, MoveXCent(p.x, r)); }
inline CRect MoveCent(const CastCRect& p, const CastCRect& r) { return MoveCent(GetCent(p), r); }

inline CRect LeftHalf(CastCRect r) { r.right = GetCent(r).x; return r; }
inline CRect RightHalf(CastCRect r) { r.left = GetCent(r).x; return r; }
inline CRect TopHalf(CastCRect r) { r.bottom = GetCent(r).y; return r; }
inline CRect BottomHalf(CastCRect r) { r.top = GetCent(r).y; return r; }


inline void FullFillRect(CDrawContext* context, CastCRect r) { r.top--; r.left--; r.right++; r.bottom++; context->fillRect(r); }
...
// return true if "r1" contains "r2" completely
{
	return r1.top <= r2.top && r1.bottom >= r2.bottom
		&& r1.left <= r2.left && r1.right >= r1.right;
}
...
public:
	SplitCRect(
		const CastCRect& rect,					// rectangle to split up
	...
	)
	{
		...
		if(sum > 0) ratio_scale = (float)pixels/sum;
	...
	// return the next rectangle split from the main one, rectangles returned from left to right or top to bottom
	CRect next()
	{
...
//-------------------------------------------------------------------------------------------------
template <class T> struct VstGuiRef : public _PtrBase<T>
//
...
// with a reference count of 1 and "setNew" will keep it the same. Watch out for some/most VST GUI methods
// that take a reference counted object (e.g. CViewContainer::addView) because they may decrement
// reference count twice on destruction!
//
...
//
{
	...
	{
		T* old_ptr = ptr;
		if(new_ptr) new_ptr->remember();
		if(old_ptr) old_ptr->forget();			// object may get deleted at this point
		ptr = new_ptr;
	}
	...
	VstGuiRef& operator = (T* src) { set(src); return *this; }
	VstGuiRef& operator = (const VstGuiRef& src) { set(src); return *this; }
...
template <class T> bool NewView(VstGuiRef<T>& view, CViewContainer* c)
	{ view.New(); return AddView(view, c); }
...
bool/*true=ok*/AlphaBlend(
	CDrawContext* dst_context,
	CastCRect dst_rect,					// destination rectangle
	const CDrawContext* src_context,// must have alpha channel & data must have been correctly pre-scaled
...
//-------------------------------------------------------------------------------------------------
class CViewDrawContext : public CDrawContext
// construct a draw context given a view (would be nice to have this as another constructor on CDrawContext)
{
public:
...
// (channels in host order)
{
	...
	void _set(unsigned long colour)
		{ *ptr = colour; }
	...
	void operator() (unsigned long r, unsigned long g, unsigned long b, unsigned long a) { set(r,g,b,a); }
...
//-------------------------------------------------------------------------------------------------
struct Ptr2dRGBA : public PtrRGBA
...
// pointer to a horizontal line of pixels with an end pointer
{
	...
	RowRGBA(Rng<unsigned long> src) { ptr = src.ptr; end = ptr+src.n; }

	bool isEnd() { return ptr == end; }
};

//-------------------------------------------------------------------------------------------------
struct ColumnRGBA : public Ptr2dRGBA
...
//-------------------------------------------------------------------------------------------------
class CContextRGBA : public CDrawContext
//
...
public:
	...
	) const
	{
		...
		#ifdef _WIN32
			BitBlt(
				(HDC)dst_context->getSystemContext(),
...
public:
	...
	// (all of these drawPoint methods access memory directly) after using any of the system drawing routines
...
inline CRect GetRect(const CContextRGBA* t) { return CRect(0, 0, t->getWidth(), t->getHeight()); }
...
//-------------------------------------------------------------------------------------------------
class DtPopupWdw : public CBaseObject
//
...
public:
	...
	void update(
		CastCRect sz,					// position & size of window, screen coordinates
	...
	void update(
		CPoint pos,					// position of window, screen coordinates
		CContextRGBA* bm,			// source bitmap to copy from
		unsigned char overall_alpha = 255	// overall alpha
	)
	{
...
//-------------------------------------------------------------------------------------------------
class DtHSlider : public CControl
//
...
public:
	...
	// init
	void setSize(
		CastCRect slide_rect,	// full rectangle extends 1/2 knob_rect either side
...
public: // override CView methods
	...
	virtual void setDirty (const bool state = true) { (volatile bool&)bDirty = state; }	///< set the view to dirty so that it is redrawn in the next idle. Thread Safe !

	// expect owner to draw
	virtual void  draw (CDrawContext* pContext) {}

	// mouse methods
	virtual CMouseEventResult onMouseDown (CPoint &mouse, const long& buttons);		///< called when a mouse down event occurs
	virtual CMouseEventResult onMouseUp (CPoint &mouse, const long& buttons);		///< called when a mouse up event occurs
	virtual CMouseEventResult onMouseMoved (CPoint &mouse, const long& buttons);	///< called when a mouse move event occurs
	virtual CMouseEventResult onMouseEntered (CPoint &mouse, const long& buttons);	///< called when the mouse enters this view
	virtual CMouseEventResult onMouseExited (CPoint &mouse, const long& buttons);	///< called when the mouse leaves this view
...
public:
	...
	{
		if(index != -1) return COptionMenu::setCurrent(index, count_separator);
		currentIndex = -1;
		return true;
	}
	...
	// allow index to be -1 which means nothing selected
	{
		if(index != -1) return COptionMenu::setValue((float)index);
		currentIndex = -1;
	}
...
public:
	...
	virtual void drawRect(CDrawContext * /*context*/, const CRect& /*update_rect*/) {}	///< called if the view should draw itself
...
public:
	...
	void update(
		...
		bool force_full_update = false	// by default, only update bitmaps if the index has changed since last time
	);
...
public:
	...
	float/*0..1*/getValue(float/*screen coord*/pixel) const
		{ return _rescale.getFrac(pixel); }
...
protected:
	...
	// active background rectangle, without border (width & height as specified in init), screen coordinates
	CRect _bg_rect;

public: // the following attempt to minimize redrawing
	// which image is being displayed (each bitmap has 2 images stacked vertically: not-selected & selected)
	int _bg_i, _knob_i;
...
class DtPopupHSlider :
	public COptionMenu,		// we have a pull down menu
...
public: // menu interface
	...
	// only valid during listener->valueChanged() >= 0 if menu item selected otherwise -1 for slider drag
	...
	//
	int/*position added*/addMenuEntry(
		float value,		// 0..1 or -ve for no associated value
		const char* txt		//
	);

	bool setMenuEntry(
		long index,			// menu index
		float value,		// 0..1 or -ve for no associated value
		const char* txt		//
	);
...
public: // override CControl methods
	...
	// note that COptionMenu calls the virtual setValue(), but direct calls to our class call setValue(float, bool), above
...
//-------------------------------------------------------------------------------------------------
class DtOwnerToggle : public CControl
//
...
public:
	...
	bool getTentativeValue() const { return _state==MOUSE_DOWN_INSIDE ? ! getBoolValue() : getBoolValue(); }
...
//-------------------------------------------------------------------------------------------------
template <class VIEW, class EVENT_TARGET> class CViewEventIntercept : public VIEW
...
public:
	#define CViewEventIntercept_PRE() \
		...
		if(_mouse_fn_before) (_event_target->*_mouse_fn_before)(&args); \

	#define CViewEventIntercept_POST() \
		args.post_call = true; \
		if(_mouse_fn_after) (_event_target->*_mouse_fn_after)(&args); \
	...
	#define CViewEventIntercept_MOUSE(METHOD, EVENT_TYPE) \
		...
		{ \
			...
			if(args.result != kMouseEventHandled) args.result = VIEW::METHOD(mouse, buttons); \
	...
	virtual bool onWheel (const CPoint &mouse, const CMouseWheelAxis &axis, const float &distance, const long &buttons)
	{
		...
		if(args.result != kMouseEventHandled)
			if(VIEW::onWheel(mouse, axis, distance, buttons)) args.result = kMouseEventHandled;
		CViewEventIntercept_POST();
