...
2. DtBlkFx calls Get_DtBlkFx function to retrieve the specific effect instance derived from _DtBlkFx class
3. DtBlkFx calls the "process" method on instance from (2)
4. Process method instantiates an Effect processing class (on the stack) and calls the "Run" template function
...
6. The run template may instantiate another MaskProcess if the previous effect is say a harmonic mask
...
The key things are:
   ...
   There are 2 levels of classes for each effect: the interface based on _DtBlkFx where there is one instance
   per effect and the actual effect processing that is derived from the internal class "ProcessBase".

   Masks and effects are both based on "ProcessBase" and calls flow downwards via the "run" method of
   "ProcessBase" that they override.

   The ProcessBase effects are instantiated on the stack, so that means you can't store any data between
   blocks in them
...
public:
  ...
  ProcessBase(FxState1_0* s) { _s = s; _b = s->_b; }
...
//*************************************************************************************************
class AmpProcess : public ProcessBase
//
...
public:
  ...
  AmpProcess(FxState1_0* s, float amp) : ProcessBase(s) { _amp = amp; }
  AmpProcess(FxState1_0* s) : ProcessBase(s) { _amp = s->temp.amp; }

  // apply scaling from bin b0 to b1
  void run(long b0, long b1)
  {
    VecPtr<cplxf, AUDIO_CHANNELS> d = _b->FFTdata();
    for(int i = b0; i <= b1; i++) d[i] *= _amp;
  }

};

//*************************************************************************************************
template <class T> class MaskProcessBase : public ProcessBase
...
public:
  ...
  MaskProcessBase(FxState1_0* s) : ProcessBase(s) {}
...
// harmonic parameters derived from a frequency and fx value
{
  ...
  // initialize
  void init(
    float/*0..1*/fx_val,	// controls harmonic type & spacing
  ...
  // initialize, expect _multiply to already be set
  {
    ...
    switch(h.i_part) {
    case 0: _offs = f; _spacing = f; break; // all harmonics
    case 1: _offs = f; _spacing = f*2; break; // odd harmonics
    case 2: _offs = f*2; _spacing = f*2; break; // even harmonics
    case 3: _offs = f*0.5f; _spacing = f; break; // in between harmonics
    }
...
//*************************************************************************************************
template <class T> class HarmMaskProcess : public MaskProcessBase<T>, public HarmParam
{
public:
  ...
  void run(long b0, long b1)
  {
    if(b0 < _min_bin) b0 = _min_bin;
    if(b1 > _max_bin) b1 = _max_bin;
    if(b0 > b1) return;
...
protected:
  ...
  // call init()
  HarmMaskProcess(FxState1_0* s) : MaskProcessBase<T>(s) {}
...
//-------------------------------------------------------------------------------------------------
template <class T> class AutoHarmMaskProcess : public HarmMaskProcess<T>
//
...
public:
  ...
  AutoHarmMaskProcess(
    ...
    float multiply,		// harmonic position multiply (1=peak is fundamental, .5=fundamental is 1/2 peak, etc)
  ...
  ) : HarmMaskProcess<T>(s)
  {
    ...
    base::init(
      fx_val,
      PeakFindFft(base::_b->FFTdata(/*left*/0) , b0, b1, /*estimate fundamental*/5.0f) // find fft peak bin
    );
  }
};

//-------------------------------------------------------------------------------------------------
template <class T> class SplitMaskProcess : public MaskProcessBase<T>
//
...
public:
  ...
  SplitMaskProcess(FxState1_0* s, T& process) : MaskProcessBase<T>(s)
  {
    base::_process = &process;
  ...
  void run(long b0, long b1)
  {
    if(_param_bin[0] <= _param_bin[1]) {
      ...
      if(v1 >= v0) base::_process->run(v0, v1);
    }
    else {
      ...
      long v0 = max(b0, _param_bin[0]);
      if(base::reverse()) {
        if(v1 >= b0) base::_process->run(b0, v1);
        if(b1 >= v0) base::_process->run(v0, b1);
      }
      else {
        if(b1 >= v0) base::_process->run(v0, b1);
        if(v1 >= b0) base::_process->run(b0, v1);
...
//-------------------------------------------------------------------------------------------------
template <class T, int SELECT_BELOW> class ThreshMaskProcess : public MaskProcessBase<T>
//
...
public:
  ...
  template <int DIR/*1=fwd, -1=rev*/> bool/*peak found*/findThreshBrk(
    float thresh_val,
    CplxfPtrPair& /*in-out*/ dat
  )
  {
    while(!dat.equal()) {
      float t = norm(*dat.a);
      if(SELECT_BELOW ? t < thresh_val : t >= thresh_val) return true;
      dat.a += DIR;
    }
    return false;
  }
  ...
  void run(long b0, long b1)
  {
    ...
    for(; !dat.equal(); dat.a++) pwr_lim(norm(*dat.a));
    ...
    if(base::reverse()) {
      ...
      while(!dat.equal()) {
        prev_dat_a = dat.a;
        dat.a--;
        if(!findThreshBrk</*DIR*/-1>(thresh_val, dat)) break;
        ...
        if(dist > _width2_bins) {
          ...
          if(v0 <= v1) base::_process->run(v0, v1);
    ...
    // run final rng
    if(v0 >= 0 && v0 <= v1) base::_process->run(v0, v1);
  }
};
...
//-------------------------------------------------------------------------------------------------
class HarmMaskFx : public FxRun1_0
// harmonic mask effect does no processing - it provides a freq mask for the next effect
{
public:
  HarmMaskFx(const char* name, bool freq_b_param_used) : FxRun1_0(name)
  {
...
{
public:
  ThreshMaskFx() : FxRun1_0("ThreshMask")
  {
    _params_used[BlkFxParam::FX_AMP] = false;
...
{
  if(prev_fx == &g_auto_harm_mask) mult = 1.0f;
  else if(prev_fx == &g_auto_subharm1_mask) mult = 0.5f;
  else if(prev_fx == &g_auto_subharm2_mask) mult = 1.0f/3.0f;
  else if(prev_fx == &g_auto_subharm3_mask) mult = 0.25f;
  else return false;
  return true;
}
...
//-------------------------------------------------------------------------------------------------
template<class T> inline bool/*ran mask*/HarmMaskRun(
  FxState1_0* s,
...
// if so, run the effect with the harmonic mask
{
  ...
  float mult;
  if(!getHarmMaskFreqMult(prev_s->temp.fft_fx, /*out*/mult)) return false;

  AutoHarmMaskProcess<T> autoharm(
    prev_s,
    end_process,
    prev_s->temp.val,	// effect value
    mult,				// harmonic multiplier
    prev_s->temp.bin[0], prev_s->temp.bin[1] // start & stop bins for search
  );
...
//-------------------------------------------------------------------------------------------------
template<class T> inline bool/*ran mask*/ThreshMaskRun(
  FxState1_0* s,
  T& end_process			// processing to perform after masking
)
{
  ...
  // threshold mask?
  if(prev_s->temp.fft_fx != &g_thresh_mask) return false;
...
//-------------------------------------------------------------------------------------------------
template<class T> inline void MaskedRun(
  FxState1_0* s,
  T& end_process
)
//
{
  ...
  if(HarmMaskRun(s, end_process)) return;

  // chk for/run threshold mask
  if(ThreshMaskRun(s, end_process)) return;
...
//
{
  ...
  DtBlkFx* b = s->_b;
  AutoHarmMaskProcess<T>  auto_mask(s, end_process, /*fx_val*/0.2499f, /*freq mult*/1.0f, /*b0*/1, /*b1*/b->_freq_fft_n/8);
...
{
public:
  FilterFx() : FxRun1_0("Filter") { _params_used[BlkFxParam::FX_VAL] = false; }
...
//*************************************************************************************************
struct ContrastProcess : public AmpProcess
...
//
{
  ...
  //
  ContrastProcess(FxState1_0* s) : AmpProcess(s)
  {
    // convert from 0..1 to -1..1
    _raise = (s->temp.val-0.5f) * 2;

    // arbitrary scaling function
    if(_raise >= 0.0f) _raise = lin_interp(_raise*_raise, 0.0f, 4.0f);
    else _raise = lin_interp(_raise*_raise, 0.0f, -0.5f);
  ...
  void run(long b0, long b1)
  {
    for(int ch = 0; ch < AUDIO_CHANNELS; ch++) {
      ...
      MatchPwr(/*scale*/AmpProcess::_amp, /*target*/in_pwr, /*current*/out_pwr, CplxfPtrPair(_b->FFTdata(ch), b0, b1+1));
    }
  }
};
...
struct SmearProcess : public AmpProcess
{
  ...
  SmearProcess(FxState1_0* s) : AmpProcess(s)
  {
    _smear = s->temp.val;
  }
...
{
public:
  SmearFx() : FxRun1_0("Smear")
    { _fillValues(/*num*/5); }

  virtual void process(FxState1_0* s) { SmearProcess smear(s); MaskedRun(s, smear); }
...
//*************************************************************************************************
struct ClipProcess : public AmpProcess
// clipping is a fraction of maximum
{
  ...
  ClipProcess(FxState1_0* s) : AmpProcess(s)
  {
  ...
  // clip bins with magnitude higher than the threshold
  {
    for(int ch = 0; ch < AUDIO_CHANNELS; ch++) {
      ...
      // find min & max pwr of channel 0
      FindMinMax<float> pwr_lim(1e30f, 1e-30f);
      for(x = fft_data; !x.equal(); x.a++) pwr_lim(norm(*x.a));
...
{
public:
  ClipFx() : FxRun1_0("Clip")
    { _fillValues(/*num*/5); }

  virtual void process(FxState1_0* s) { ClipProcess clip(s); MaskedRun(s, clip); }
...
//*************************************************************************************************
//-------------------------------------------------------------------------------------------------
static float/*octaves shift*/ParamToOctave(float/*0..1*/v) { return lin_interp(v, -3.0f, 3.0f); }

//-------------------------------------------------------------------------------------------------
class ShiftProcess : public AmpProcess
//
{
public:
  // max bin
  ...
  //
  ShiftProcess(FxState1_0* s) : AmpProcess(s)
  {
    _phase.init(/*AmpProcess::*/_b);

    // if amp is less than 1 then we're in mix-mode
    if(_amp < 1.0f) _mix_amp = 1.0f - _amp;
    else _mix_amp = 0.0f;
  ...
  void run(long dst_bin, long src_bin, long n_bins)
  {
    ...
    if(src_bin+n_bins > _max_bin) n_bins = _max_bin - src_bin;
    ...
    if(dst_bin+n_bins > _max_bin) n_bins = _max_bin - dst_bin;
    ...
    if(_reverse) {
      ...
      for(int ch = 0; ch < AUDIO_CHANNELS; ch++) {
        ...
        for(CplxfPtrPair xs(_b->FFTdata(ch), src_bin+n_bins-1, src_bin-1); !xs.equal(); xs.a--, xd--)
          run(xd, xs.a);
      }
    }
    else {
...
//-------------------------------------------------------------------------------------------------
// fixed shift amounts
float g_shift_value_presets_fixed[] = {
  0.5f,
...
{
  if(!idx_within(idx, run->_value_presets)) return curr_val;

  // value from array
  float v = run->_value_presets[idx].val;

  // first menu item sets directly
  if(idx < NUM_ELEMENTS(g_shift_value_presets_fixed)) return v;
...
public:
  ...
  virtual void process(FxState1_0* s) { ShiftProcess shift(s); MaskedRun(s, shift); }
...
//*************************************************************************************************
class ConstShiftProcess : public AmpProcess
...
public:
  ...
  ConstShiftProcess(FxState1_0* s) : AmpProcess(s)
  {
    // shift freq by this much
    _frq_shift =
      paramToHz(s->temp.val)/_b->getSampleRate() * _b->_freq_fft_n;
...
public:
  ...
  ConstShiftFx() : FxRun1_0("ConstShift")
  {
    // shift amounts
    float values[] = {
      0.321054f, 0.389299f, 0.473659f, 0.476198f, 0.478758f, 0.481339f, 0.483942f, 0.486566f,
      0.489211f, 0.491877f, 0.494564f, 0.497272f, 0.500000f, 0.502728f, 0.505436f, 0.508123f,
      0.510789f, 0.513434f, 0.516058f, 0.518661f, 0.521242f, 0.523802f, 0.526341f, 0.610701f,
      0.678946f
    };
...
public:
  ...
  virtual void process(FxState1_0* s)
  {
    ...
    AutoHarmMaskProcess<AmpProcess> harm(s, amp, /*value*/s->temp.val, /*multiplier*/1.0f, /*b0*/0, /*b1*/b->_freq_fft_n/8);
...
//*************************************************************************************************
class ResizeProcess : public AmpProcess
...
public:
  ...
  ResizeProcess(FxState1_0* s, FixPoint<12> frq_mult, bool conjugate_mode) : AmpProcess(s)
  {
...
public:
  struct Param : public SplitParam<2>
  {
    ...
    Param(float param_val) : SplitParam<2>(param_val) { f_part = powf(lin_interp(f_part, -2.0f, 2.0f), 3); }
  };

  ResizeFx() : FxRun1_0("Resize")
  {
    float values[] = {
      0.000000f, 0.051575f, 0.092510f, 0.125000f, 0.150787f, 0.171255f, 0.191980f, 0.308020f,
      0.328745f, 0.349213f, 0.375000f, 0.407490f, 0.448425f, 0.4999f, 0.500000f, 0.551575f,
      0.592510f, 0.625000f, 0.650787f, 0.671255f, 0.691980f, 0.808020f, 0.828745f, 0.849213f,
      0.875000f, 0.907490f, 0.948425f, 1.0f
    };
    _fillValues(TO_RNG(values));
  }
  ...
  virtual void process(FxState1_0* s)
  {
    ...
    if(p.i_part) conj_mode = !conj_mode;
    if(p.f_part < 0) conj_mode = !conj_mode;
...
public:
  ...
  void prepare(HarmMaskProcess<HarmMatchProcess>* parent)
  {
    ...
    if(harm0_pwr <= 0) return;

    for(int ch = 0; ch < AUDIO_CHANNELS; ch++) {
      _pwr_scale[ch] = GetPwr(_b->FFTdata(ch), f0, f1)/harm0_pwr;
      if(_copy_mode) _pwr_scale[ch] *= _amp*_amp;
    }
  }
  ...
  ) : AmpProcess(s)
  {
    ...
    for(int ch = 0; ch < AUDIO_CHANNELS; ch++) _pwr_scale[ch] = 1.0f;
  ...
  void run(long b0, long b1)
  {
    ...
    if(curr_harm < 0 || curr_harm >= _n_harms) return;
...
public:
  HarmData _data;
  HarmMatchFx(const char* name, HarmData data) : FxRun1_0(name)
  {
    _data = data;
  ...
  void prepare(HarmMaskProcess<HarmShiftProcess>* parent)
  {
    ...
    if(!_repitch_mode) {
      ...
      if(fx_val >= 0.7f) {
        ...
        PeakFindFft peak_r(_b->FFTdata(/*left*/1), /*b0*/1, /*b1*/_b->_freq_fft_n/8, /*estimate fundamental*/5.0f);
        ...
        frq_mult *= powf(
          ...
          lin_scale(
            fx_val,
            0.7f, 1.0f,		// in min & max
            -24.0f, 24.0f	// out min & max notes
...
{
public:
  HarmShiftFx() : FxRun1_0("HarmShift")
    { InitShiftValuePresets(this); }
...
{
public:
  HarmRepitchFx() : FxRun1_0("HarmRepitch")
  {
  ...
  virtual float getValue(int idx, float curr_val)
  {
    if(!idx_within(idx, _value_presets)) return curr_val;

    //
    float v = _value_presets[idx].val;

    // first menu item sets directly
    if(idx < NUM_ELEMENTS(g_repitch_value_presets_fixed)) return v;
  ...
  {
    #ifdef STEREO
      if(val >= 0.7f) return text << sprf("right%+.2fnotes", lin_scale(val, 0.7f, 1.0f, -24, 24));
    #endif
...
public:
  ...
  ResampleProcess(FxState1_0* s) : AmpProcess(s)
  {
...
public:
  ...
  virtual void process(FxState1_0* s) { ResampleProcess resample(s); MaskedRun(s, resample); }
...
public:
  ...
  static float getSegs(float params_val) { return min(params_val*(399.0f/.875f)+1.0f, 400.0f); }
  ...
  virtual void process(FxState1_0* s)
  {
    ...
    if(voc_amp > 0.0f) {
      ...
      while(1) {
        ...
        if(dst.b > dst_end) break;

        // get the power from the src segment (or from src start bin if empty)
        float src_pwr;
        if(src.equal()) src_pwr = norm(*src);
        else src_pwr = GetPwr(src);
        ...
        for(;!dst.equal(); dst.a++) *dst = (*dst) * dst_scale;
    ...
    if(mult_amp > 0.0f) {
      ...
      if(bin[0] > bin[1]) swap(bin[0], bin[1]);
  ...
  virtual Rng<char> /*updated*/ dispVal(FxState1_0*, Rng<char> /*out*/text, float/*0..1*/val)
  {
    ...
    if(mult > 0.0f) return text << spr_percent(mult) << " multiply";
...
public:
  ...
  HarmMatch2Process(FxState1_0* s, int src_ch, int dst_ch) : AmpProcess(s)
  {
    _src_ch = src_ch;
    _dst_ch = dst_ch;
...
public:
  bool _RL_mode;
  HarmMatch2Fx(const char* name, bool RL_mode) : FxRun1_0(name)
  {
    InitHarmValuePresets(this);
    _RL_mode = RL_mode;
  }
  ...
  virtual void process(FxState1_0* s)
  {
    ...
    if(_RL_mode) swap(src_ch, dst_ch);

    HarmMatch2Process match(s, src_ch, dst_ch);

    // find peaks in src & dst data
    Array<long, 2> dst_bin = s->temp.bin, src_bin;
    if(dst_bin[0] > dst_bin[1]) swap(dst_bin[0], dst_bin[1]);
    src_bin = dst_bin;

    float fx_val = s->temp.val;

    // chk whether the previous effect is a harmonic mask, if so use its params to control the source
    FxState1_0* prev_s = s->prevFxState();
    if(prev_s) {
      if(getHarmMaskFreqMult(prev_s->temp.fft_fx, /*out*/ match._src_harm._multiply)) {
        ...
        if(src_bin[0] > src_bin[1]) swap(src_bin[0], src_bin[1]);
        fx_val = prev_s->temp.val;
      }
    }

    float src_peak = PeakFindFft(b->FFTdata(src_ch), src_bin[0], src_bin[1], /*estimate fundamental*/5.0f);
    match._src_harm.init(fx_val, src_peak);

    float dst_peak = PeakFindFft(b->FFTdata(dst_ch), dst_bin[0], dst_bin[1], /*estimate fundamental*/5.0f);
...
//*************************************************************************************************
struct CrossMixProcess : public AmpProcess
//
...
//
{
  ...
  CrossMixProcess(FxState1_0* s) : AmpProcess(s)
  {
    _bins_processed = 0;
    ...
    _src_ch = 0;
    _dst_ch = 1;
    if(val.i_part) swap(_src_ch, _dst_ch);
  ...
  void run(long b0, long b1)
  {
    ...
    for(dst = dst_; !dst.equal(); src++, dst.a++) {
      ...
      dst_pwr += norm_dst;
      *dst = polar_to_cplxf(
        powf(norm_src, _raise_src) * powf(norm_dst, _raise_dst),
  ...
  void done()
  {
    // determine power correction by linearly interpolating using mix ratio and number of bins processed
...
//*************************************************************************************************
struct WeirdMixProcess : public AmpProcess
//
//
{
  ...
  WeirdMixProcess(FxState1_0* s) : AmpProcess(s)
  {
    _bins_processed = 0;
  ...
  // internal run processing
  {
    ...
    // curve "length" normalizing multipliers (originally this really was length but now it's something else)
    ...
    while(!curr_out.equal()) {
      ...
      else out = lerp_val * AmpProcess::_amp;
  ...
  // run called by the framework
  {
    ...
    if(mix_limit[1] < mix_limit[0]) swap(mix_limit[0], mix_limit[1]);
    ...
    while(dst_bin < n_bins) {
      ...
      int dst_stop_bin = RndToInt(
        lin_interp(mix_ratio, (float)(seg_in[0].b-data_in[0]), (float)(seg_in[1].b-data_in[1]))
      );

      // make sure that at least one output bin is processed
      if(dst_stop_bin <= dst_bin) dst_stop_bin = dst_bin+1;

      // work out step size for each channel
      float
        dst_len_f = (float)(dst_stop_bin-dst_bin)+1,
        src_bin_f[/*channel*/2],
        in_stp[/*channel*/2];
  ...
  void run(long b0, long b1)
  {
    ...
    if(_mode) runMode1(n_bins, fft_data, temp_buf);
    else runMode0(n_bins, fft_data, temp_buf);
...
{
  if(idx < 0 || idx >= g_num_fx_1_0) return &g_no_fx;
