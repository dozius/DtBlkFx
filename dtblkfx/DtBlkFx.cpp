...
// note, expect any allocation failures to throw an exception
{
  ...
  for(i = 0; i < AUDIO_CHANNELS; i++) {
    _chan[i].x0.resize(_x0_sz+MAX_FFT_SZ);	// input buffer with extra space at end to unwrap data for processing
    _chan[i].x1.resize(MAX_FFT_SZ/2+32*2);	// FFT'd complex data with space either side for shift overflow
    _chan[i].x2.resize(MAX_FFT_SZ+32*2);	// inverse FFT & temporary buffer with space either side for shift overflow
    _chan[i].x3.resize(_x3_sz);				// output buffer (length is arbitrary, as long as > MAX_FFT_SZ plus a few)
  }
  ...
  if(AUDIO_CHANNELS == 2) setUniqueID ('h526');
  else setUniqueID ('g319');
...
//
{
  ...
  for(unsigned i = 0; i < wdw_n; i++) {
    float r[4] = {
      0.5f-0.5f*cosf(a[0]), sinf(a[1]), a[2], a[3]
    };
    for(int j = 0; j < 4; j++) {
...
// only save using the latest chunk version
{
  ...
  int payload_bytes = PackedBytesPerVstProgram(BlkFxParam::TOTAL_NUM)*(num_programs+1/*curr params*/);
  _chunk_data.resize(/*tag field*/4 + /*vers field*/4 + /*n programs field*/4 + /*curr program field*/4 + payload_bytes);
  ...
  for(i = 0; i < BlkFxParam::TOTAL_NUM; i++) curr_program.params[i] = _params.getInputNoForced(i);
...
// virtual, override AudioEffect
{
  ...
  if(vers == 100 || vers == 101) {
    ...
    if(vers==101) num_params = 4+5*8; // 8 effects
    ...
    if(isPreset) {
      ...
      if(i > 0 && i <= max_num_programs && strcmp(_program[i-1].getName(), "> reset current <") == 0)
...
// called by vst-host to set a program
{
  ...
  for(int i = 0; i < BlkFxParam::TOTAL_NUM; i++) setParameter(i, currProgram().params[i]);
}
...
// called by vst-host to get the name of a program by category
{
  ...
  if(index < 0 || index >= (int)_program.size()) return false;
...
struct MyInfo : public VstTimeInfo
{
  void dbgprint() {
    LOG("", "VstTimeInfo"
      ...
      << VAR(cycleStartPos)
      << VAR(cycleEndPos)
      << VAR(timeSigNumerator)
      << VAR(timeSigDenominator)
      << VAR(smpteOffset)
      << VAR(smpteFrameRate)
      << VAR(samplesToNextClock)
      << VAR(flags)
    );
  }
};
...
//
{
  ...
  time_fft_n = (int)(
    (float)freq_fft_n*
    lin_interp(
      get(&GetInput, _blk_shoulder_frac_param),
      1.0f, .25f
    )
  );
  ...
  if(time_fft_n > delay_n) {
    ...
    if(time_fft_n > delay_n) time_fft_n = delay_n;
  }
}
...
//
{
  if(!force && _curr_samp_abs - _prev_poll_abs < _samps_per_poll) return;
  ...
  //LOG("", "pollUpdate" <<VAR(_samps_per_beat)<<VAR(ppq_pos)<<VAR(ppq_frac)<<VAR(_beat_start_abs));
...
{
  // safety
  if(index < 0 || index >= BlkFxParam::TOTAL_NUM) return;
...
// called by vst-host to get param value
{
  ...
  if(index < 0 || index >= TOTAL_NUM) return 0.0f;
...
// called by vst-host to get name of param
{
  ...
  // try global params
  switch(p.glob_param) {
  case MIX_BACK: str << "Mixbk"; return;
  case DELAY:    str << "Delay"; return;
  case FFT_LEN:  str << "BlkSz"; return;
  case OVERLAP:  str << "Ovrlp"; return;
  }
  // try fx set
  switch(p.fx_param) {
  case FX_FREQ_A: str << p.fx_set << ".FrqA"; return;
  case FX_FREQ_B: str << p.fx_set << ".FrqB"; return;
  case FX_AMP:    str << p.fx_set << ".Amp"; return;
  case FX_TYPE:   str << p.fx_set << ".Fx"; return;
  case FX_VAL:    str << p.fx_set << ".Val"; return;
  }
}
...
//-------------------------------------------------------------------------------------------------
bool/*true=printed*/DtBlkFx::getParamDisplayGlobal(BlkFxParam::SplitParamNum& p, float v, CharRng text)
{
  using namespace BlkFxParam;
  ...
  switch(p.glob_param) {
    ...
    case DELAY: {
      if(_delay_param.vstParamIdx() != DELAY) return false;
    ...
    case FFT_LEN: {
      if(_fft_len_param.vstParamIdx() != FFT_LEN) return false;
    ...
    case OVERLAP: {
      bool printed = false;
      if(
        _beat_sync_param.vstParamIdx() == OVERLAP && _beat_sync_param(v) > 0.5f ||
...
// called by vst-host to convert a param to text
{
  ...
  if(p.ok) {
    if(getParamDisplayGlobal(p, v, str)) return;
    if(p.fx_set >= 0 && _fx1_0[p.fx_set].getParamDisplay(p, v, str)) return;
  }
...
// assume that we don't get blocks bigger than our buffer size (several seconds worth of data)
{
  ...
  while(buf_n) {
    ...
    if(overflow >= 0) _x0_n_past_end = overflow;
...
// any of them
{
  ...
  while(1) {
    ...
    if(!_params.nextOutParam()) return;
  }
}
...
//
{
  if(!_params_need_processing) return;
  _params_need_processing = false;

  // check whether we should sync with a param
  if(_params_state == PARAMS_CHK_SYNC) paramsChkSync();
  ...
  _time_fft_n = (int)((float)_freq_fft_n*lin_interp(
      get(&GetInterp, _blk_shoulder_frac_param),
      1.0f, .25f
    )
  );
  ...
  // make sure dest position isn't in data that we've already output (i.e. behind current sample pos)
...
// internal method
// prepare input blk
// variables update: _x0_i, _x0_n, _blk_samp_abs, _data_pre_x0_n, _time_fft_n, _freq_fft_n, _extra_data
{
  ...
  if(_extra_data >= 0) {
    ...
    if(_data_pre_x0_n < 0) _data_pre_x0_n = 0;
  }
  else {
...
//
{
  ...
  if(x0_xform_i < 0) x0_xform_i += _x0_sz;

  // find the amount of shoulder data that we can apply a window function to (window is symmetrical)
  int shoulder_n = min(/*right*/_data_pre_x0_n, /*left*/_freq_fft_n-_time_fft_n-_data_pre_x0_n);
  ...
  if(shoulder_n > /*arbirary*/12) {
    ...
    if(_time_fft_n < 0) _time_fft_n = 0;
...
//
{
  ...
  if(_fadein_n >= 0) {
    ...
    //LOG("", "mixToOutputBuffer gap" << VAR(zero_o) << VAR(zero_n) << VAR(prev_fade_n) << VAR(prev_fade_o));
  }

  // update the end position of data in x3 if it is extended (this is slightly early but that
  // doesn't matter)
  if(extend_x3 > 0) _x3_end_abs = dst_fft_end_abs;

  // ensure fade in is not too big
  if(_fadein_n > _time_fft_n) _fadein_n = _time_fft_n;
...
// process the FFT'd data (x1) using the fx_params
{
  ...
  for(i = 0; i < BlkFxParam::NUM_FX_SETS; i++) _fx1_0[i].prepare();
  for(i = 0; i < BlkFxParam::NUM_FX_SETS; i++) _fx1_0[i].process();
  ...
  for(i = 0; i < AUDIO_CHANNELS; i++) {
    ...
    if(pwr_scale > 1e30) pwr_scale = 1.0f;			// too big
    if(pwr_scale < 1e-30) pwr_scale = 0.0f;			// too small (or worse, negative)
...
)
{
  ...
  while(data_out < data_out_end) {
    while(lerp_x <= out_idx) {
      ...
      if(data_in[a] >= data_in_last[a]) break;
...
// perform ifft & mix to output
{
  ...
  for(int i = 0; i < AUDIO_CHANNELS; i++) {
    ...
    if(_mixback <= 0.0f) mixToX3(P1Src(x2, x2_scale), i);
...
//
{
  ...
  _next_blk_fwd_n = BlkFxParam::getBlkShiftFwd(
    get(&GetInterp, _overlap_param),
    _time_fft_n
  );
  ...
  if(beat_sync > /*arbitrary*/16) {
    ...
    //LOG("", "nextBlk, blksync" <<VAR(_next_blk_fwd_n) <<VAR(sync_fwd) <<VAR(_blk_samp_abs) <<VAR(next_blk_samp_abs)<<VAR(beats)<<VAR(sync_abs)<<VAR(overlap_param));
    if(sync_fwd > _min_blk_fwd_samps && sync_fwd <= _next_blk_fwd_n) _next_blk_fwd_n = sync_fwd;
  }
...
//
{
  ...
  if(zero_n <= 0 || _x3_is_clear) return;
...
// internal method
{
  ...
  while(1) {
    ...
    if(_extra_data <= 0) break;
  }

  //
  ASSERTX(_buf_end_abs == _curr_samp_abs+buf_n, VAR(_buf_end_abs) << VAR(_curr_samp_abs) << VAR(buf_n));
