...
inline DtBlkFx* GlobalCtrl::blkFx() { return _gui->blkFx(); }
...
///< called if the view should draw itself
{
	...
	if(ShouldDrawView(update_rect, _mixback)) {
		...
		temp_bm->setFontColor(_mixback->getState()==_mixback->INACTIVE ? normal_txt_col:focus_txt_col);
	...
	if(ShouldDrawView(update_rect, _pwrmatch)) {
		...
		temp_bm->setFontColor(_pwrmatch->getState()==_pwrmatch->INACTIVE ? normal_txt_col:focus_txt_col);
	...
	if(ShouldDrawView(update_rect, _overlap)) {
		...
		temp_bm->setFontColor(_overlap->getState()==_overlap->INACTIVE ? normal_txt_col:focus_txt_col);
	...
	if(ShouldDrawView(update_rect, _blksync)) {
		...
		temp_bm->setFontColor(_blksync->getState()==_blksync->INACTIVE ? normal_txt_col:focus_txt_col);
	...
	temp_bm->blend(context, /*dst*/update_rect, /*src*/rel_update_rect, /*overall alpha*/255, /*use src alpha*/false);
}
...
// note that only the "f_part" of blk delay is stored in the menu
{
	...
	if(blkFx()->getSampleRate() != _sample_rate || blkFx()->getSampsPerBeat() != _samps_per_beat)
	{
		...
		// beat presets
		float beat_presets[] = { .25f, 1/3.0f, 0.5f, 2/3.0f, 0.75f, 1.0f, 1.5f, 2.0f, 3.0f, 4.0f, 5.0f, 6.0f, 7.0f, 8.0f };
	...
	if(_delay_units == BlkFxParam::Delay::MSEC) {
		...
		str << sprf("%.2f beats", delay_beats)
			<< "(" << (int)delay_samps << " samps, "
		...
		str << sprnum(delay_secs, 1e-3f) << "sec, "
			<< (int)delay_samps << " samps, "
	...
	str << sprnum(fft_secs, 1e-3f) << "sec, "
		<< fft_samps << " samps, "
		<< sprf("(%.2f beats)", fft_beats);

	_delay->setMenuEntry(/*idx*/1, /*value*/BlkFxParam::Delay::msec(fft_secs*1000.0f).f_part, str);
