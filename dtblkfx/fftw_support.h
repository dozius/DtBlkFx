...
inline fftwf_complex* to_fftwf_complex(cplxf* t) { return &(t->data); }
	...
	namespace FFTWf {
		inline void destroy_plan(fftwf_plan p) { fftwf_destroy_plan(p); }
		inline void execute_dft_c2r(const fftwf_plan p, fftwf_complex*i, float*o) { fftwf_execute_dft_c2r(p, i, o); }
		inline void execute_dft_r2c(const fftwf_plan p, float*i, fftwf_complex*o) { fftwf_execute_dft_r2c(p, i, o); }
		inline void free(void *p) { fftwf_free(p); }
		inline void *malloc(size_t n) { return fftwf_malloc(n); }
		inline fftwf_plan plan_dft_c2r_1d(int n,fftwf_complex*i,float*o,unsigned flags) { return fftwf_plan_dft_c2r_1d(n, i, o, flags); }
		inline fftwf_plan plan_dft_r2c_1d(int n,float*i,fftwf_complex*o,unsigned flags) { return fftwf_plan_dft_r2c_1d(n, i, o, flags); }
	};
...
//-------------------------------------------------------------------------------------------------
template<class T> struct ScopeFFTWfMalloc : public _PtrBase<T>
//
...
//
{
	...
	{
		if(base::ptr) FFTWf::free(base::ptr);
		if(n_elements) {
...
//-------------------------------------------------------------------------------------------------
struct ScopeFFTWfPlan : public _PtrBase<fftwf_plan_s>
//
// wrapper for plan
// note single ownership, use "Steal()" to transfer ownership
// plan is destroyed when out of scope
{
	...
	void Destroy() { if(base::ptr) FFTWf::destroy_plan(ptr); }
	ScopeFFTWfPlan(fftwf_plan plan = NULL) { base::ptr = plan; }
	ScopeFFTWfPlan& operator = (fftwf_plan  plan) { Destroy(); base::ptr=plan; return *this; }
...
public:
	...
	void init(
		long blk_samp_abs,	// absolute sample starting position of block
...
{
	float pwr = 0.0f;
	for(; !x.equal(); x.a++) pwr += norm(*x);
	return pwr;
}
inline float/*pwr*/GetPwr(cplxf* x, long b0, long b1 /*b0 <= b1*/) { return GetPwr(CplxfPtrPair(x, b0, b1+1)); }
inline float/*pwr*/GetPwr(cplxf* x, long n_bins) { return GetPwr(CplxfPtrPair(x, n_bins)); }

//-------------------------------------------------------------------------------------------------
inline float/*amp*/MatchPwr(
	float amp,
	double target_pwr,
	double curr_pwr
)
...
{
	double s = amp;
	if(target_pwr <= 0) return 0;
	if(curr_pwr > 0) s *= sqrt(target_pwr/curr_pwr);

	// limit the scaling
	if(s > 1e30) return 1.0f;
	if(s < 1e-37) return 0.0f;
	return (float)s;

}
//-------------------------------------------------------------------------------------------------
inline void MatchPwr(
	float amp,
	double target_pwr,
	double curr_pwr,
...
//*************************************************************************************************
template <int CHANNELS_> struct CplxfTmp
//
...
//
{
	...
	VecPtr<cplxf, CHANNELS>
		_data,// corresponding data (src/dst)
	...
	{
		for(int i = first_bin; i <= last_bin; i++) _tmp[i] = 0.0f;
	}
	...
	void flushTmp(int first_bin, int last_bin)
	{
		if(first_bin <= _tmp_first_bin) {
			if(last_bin < _tmp_first_bin) return; // don't have this data to flush

			first_bin = _tmp_first_bin;
			_tmp_first_bin = last_bin+1;
		}
		else if(last_bin < _tmp_last_bin) return; // can't segment tmp buffer

		if(last_bin >= _tmp_last_bin) {
			if(first_bin > _tmp_last_bin) return; // don't have this data to flush
			last_bin = _tmp_last_bin;
			_tmp_last_bin = first_bin-1;
		}
		else if(first_bin > _tmp_first_bin) return; // can't segment tmp buffer

		// flush the buffer
		for(int i = first_bin; i <= last_bin; i++) _data[i] += _tmp[i];
	}
	...
	void forceDirect(int src_first_bin, int src_last_bin)
	{
		...
		if(_reverse_direction) flushTmp(src_last_bin+1, _tmp_last_bin);
		else flushTmp(_tmp_first_bin, src_first_bin-1);
	}
...
//*************************************************************************************************
template <int CHANNELS> struct FrqShiftFft
//
...
public:
	...
	template <int CONJ> void run(
		int src_first_bin,			// start bin
	...
	)
	{
		...
		if(src_last_bin < src_first_bin) return;
		...
		// otherwise let overlap buffer decide
			...
			_buf.prepare(
				src_first_bin, src_last_bin,
				dst_first_bin.getRound()-FFTFracShift::W_OFFS, dst_last_bin.getRound()+FFTFracShift::W_OFFS
			);

		else 
			_buf.prepare(
				src_first_bin, src_last_bin,
				dst_last_bin.getRound()-FFTFracShift::W_OFFS, dst_first_bin.getRound()+FFTFracShift::W_OFFS
			);
		...
		if(shift_stp == 1) {
			...
			// scaling shift
			for(int src_bin = src_last_bin; src_bin >= src_first_bin; src_bin--) {
				Slice<cplxf, CHANNELS> m = getSrc<CONJ>(src_bin) * _dst_amp * _phase_corr(dst_bin-src_bin);
				_buf.data(src_bin) *= _src_amp;
				FFTFracShift::add(_buf.dst(), dst_bin, m);
				dst_bin -= shift_stp;
			}
...
public: // internals
	...
	{
		if(CONJ) return conj(_buf.data(idx));
		return _buf.data(idx);
	}

	void procSrcOnly(int first_bin, int last_bin)
	{
		for(int i = first_bin; i <= last_bin; i++) _buf.data(i) *= _src_amp;
	}
};
...
//
{
	...
	void operator() (cplxf* fft, int b0, int b1 /* b1 >= b0, b1 < fft_len/2 */, float estimate_fundamental=1.0f);
