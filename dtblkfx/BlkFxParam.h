...
namespace BlkFxParam
{
	...
	inline int paramOffs(int fx_set) { return NUM_GLOBAL_PARAMS+fx_set*NUM_FX_PARAMS; }
	...
	struct SplitParamNum
	{
		...
		SplitParamNum(int param_num)
		{
			...
			fx_set = -1;
			ok = false;
			if(param_num < 0 || param_num >= TOTAL_NUM) return;
	...
	{
		return limit_range(
			mixback_param<=0.5f ? mixback_param*2.0f : (1.0f-mixback_param)*(1.0f/0.499f),
			0.0f, 1.0f
		);
	}
	...
	{
		return pwr_match?
			limit_range(mixback_frac*0.5f, 0.0f, 0.5f) :
			limit_range(1.0f-mixback_frac*0.499f, 0.501f, 1.0f);
	}

	// --- DELAY -----------------------------------------------------------------------------------------------
	struct Delay : public SplitParam<2>
	// f_part is converted to delay units (beats or msec)
	{
		...
		void setAmount(Units units, float amount /* beats or msec */)
		{
			setUnits(units);
			f_part = limit_range(
				amount * (i_part==MSEC ? 1.0f/6000.0f : 2.0f*TICKS_PER_BEAT/255.0f),
				0.0f, 1.0f
			);
		}
	...
	// given vst param for fft_len, return fft plan number
	{
		...
		return limit_range(
			(int)(fft_len_param*255.0f/4.0f-1.5f),
			0,
			NUM_FFT_SZ-1
		);
	}
	...
	// turn plan index into fft length
	{
		// scaling is compatible with 1.0 version
		return limit_range(
			((float)plan_index+2.0f)*(4.0f/255.0f),
			0.0f, 1.0f
		);
	}
	...
	{
		return sync?
			limit_range((2.0f-overlap_part)*0.5f, 0.501f, 1.0f) :
	...
	inline float noteSpan() { return 255.0f*0.5f; }

	// number of octaves spanned in freq param
	inline float octaveSpan() { return noteSpan()/12.0f; }

	// fraction of freq param corresponding to 1 octave
	inline float octaveFrac() { return 1.0f/octaveSpan(); }

	// fraction of freq param corresponding to 1 note
	inline float noteFrac() { return 1.0f/noteSpan(); }
	...
	inline float/*param 0..1*/getEffectTypeInv(long index) { return (float)(index*8+4)/255.0f; }
	...
	// turn db into param
	inline float/*0..1*/getAmpParam(float db/*-60 .. 40*/) { return limit_range(db*0.01f + 0.6f, 0.0f, 1.0f); }

	// get 0dB point of amp param
	inline float getAmpParam0dB() { return getAmpParam(0); }
	...
	// if mix_mode is true and amp is less than 0dB point, linear
	{
		...
		if(mix_mode && param < getAmpParam0dB()) return param * (1.0f/getAmpParam0dB());
